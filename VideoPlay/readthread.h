...
public:
    ...
    bool status();   //判断当前线程是够在播放视频
    void setPlaybackSpeed(int speed);  //通过此函数在外部修改播放速度
    //设置位置
    void setVideoPosition(qint64 position);
    qint64 getcurrenttime();                 //获取现在视频播放时间
    qint64 gettotaltime();                  //获取视频总时间
...
private:
    ...
    int m_playbackSpeed = 1;                    // 控制视频倍速播放
};
