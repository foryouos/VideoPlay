...
bool ReadThread::status()
{
    return m_play;
}

void ReadThread::setPlaybackSpeed(int speed)
{
    m_playbackSpeed = speed;
}

void ReadThread::setVideoPosition(qint64 position)
{
    m_videoDecode->setPosition(position);
}

qint64 ReadThread::getcurrenttime()
{
    return m_videoDecode->getcurrenttime();
}

qint64 ReadThread::gettotaltime()
{
    return m_videoDecode->gettotaltime();
}
...
#else
            int sleepTime = int((m_videoDecode->pts() - m_etime2.elapsed()) / m_playbackSpeed); //设置倍速播放
            sleepMsec(sleepTime);
